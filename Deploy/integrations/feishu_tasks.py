...
def load_local_env(force=False):
    ...
    global _ENV_LOADED
    if os.getenv("MINSIGHT_CONFIG_MODE", "development").strip().lower() in {"production", "prod", "deploy"}:
        _ENV_LOADED = True
        return None
...
def build_task_sink(mode=None, runner=None):
    ...
    if selected in {"lark_cli", "lark-cli", "cli"}:
        ...
        if _env_enabled("MINSIGHT_FEISHU_RESOLVE_ASSIGNEE", default=False):
            try:
                from .feishu_contacts import LarkCliContactResolver
            except ImportError:  # pragma: no cover - direct Lab script path
